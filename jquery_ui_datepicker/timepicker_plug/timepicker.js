...
// OK datum einlesen
// OK beim ‰ndern des input feldes widget anpassen / neu einlesen
...
//								close:	'Close'			
				...
				function renderPickerPlug(b24Hour_,lang_) {
					...
					if (!$('#pickerplug').length) {
						...
						htmlins += '</li>';
						htmlins += '<li>';
						htmlins += '<div id="timepicker" class="ui-corner-all">';
						...
						//
						htmlins += '	<span class="delim" >:</span>';
						htmlins += '	<span class="selSecs">00</span>';
						//
						...
						//
						htmlins += '	<li>';
						htmlins += '        <h4 id="text_second"></h4>';				
						htmlins += '        <div id="secondSlider" class="slider"></div>';
						htmlins += '	</li>';
						//
						htmlins += '</ul>';
						htmlins += '</div>';
	 ...
	 // Slider
						...
						$('#secondSlider').slider({
							...
							step: 1,
							slide: function(event, ui) {                   
								writeDate(writeTime(ui.value,'second'),'time');                                           
							},
							change: function(event, ui) {
								$('#tpSelectedTime .selSecs').effect('highlight', 1000);
							}
						});
		...
		//Inline editor bind
					...
					//}
		//Inline editor bind
					$('#tpSelectedTime .selSecs').keyup(function(e){
						if((e.which <= 57 && e.which >= 48) && ($(this).text() >=0 && $(this).text() <=59 ) ){
						//console.log("Which: "+e.which);
					   $('#secondSlider').slider('value', parseInt($(this).text()));
						//console.log("Val: "+parseInt($(this).val()))
						}else{
							$(this).text($(this).text().slice(0, -1));
						}
					...
					$('#text_second').text(loadedLang['second']);
				...
				function parseTime (obj) {
					...
					var second 	= time[2] || '00';
				
					writeTime(hour,'hour');
					writeTime(minute,'minute');
					writeTime(second,'second');

					$('#hourSlider').slider('option', 'value', hour);
					$('#minuteSlider').slider('option', 'value', minute);
					$('#secondSlider').slider('option', 'value', minute);	
					if ($('#pickerplug').data('lastdate') == '0000-00-00') {
				...
				function writeTime(fragment,type) {
					...
					switch (type) {
						case 'hour':
	                    	...
	                    	time = time+':'+$('#tpSelectedTime .selSecs').text();
							break;
						case 'minute':
	                    	minutes = ((fragment < 10) ? '0' :'') + parseInt(fragment,10);
	                    	$('#tpSelectedTime .selMins').text(minutes);
	                   
	                        time = $('#hourSlider').slider('option', 'value')+':'+minutes+ ':'+$('#secondSlider').slider('option', 'value');  						
							break;
						case 'second':
	                    	seconds = ((fragment < 10) ? '0' :'') + parseInt(fragment,10);
	                    	$('#tpSelectedTime .selSecs').text(seconds);
	                   
	                        time = $('#hourSlider').slider('option', 'value')+':'+ $('#minuteSlider').slider('option', 'value')+':'+seconds;  						
							break;	
					}
					return time;
				}				
